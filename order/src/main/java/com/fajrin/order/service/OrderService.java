...
public class OrderService {
    ...
    public List<ResponseTemplate> getOrderWithProdukById(Long id){
        ...
        Produk produk = restTemplate.getForObject("http://localhost:9001/api/v1/produk/" + order.getProdukId(), Produk.class);
    ...
    public List<ResponseTemplate> getOrderWithPembayaranById(Long id){
        List<ResponseTemplate> responseList = new ArrayList<>();
        Order order = getOrderById(id);
        Pembayaran pembayaran = restTemplate.getForObject("http://localhost:9003/api/v1/pembayaran/" + order.getPembayaranId(), Pembayaran.class);
        ResponseTemplate vo = new ResponseTemplate();
        vo.setOrder(order);
